...
import { readableStreamToString, VisierModelQueryError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new VisierModelQueryError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new VisierModelQueryError(e, e.response?.data)
                }
                if (innerError instanceof VisierModelQueryError) {
                    // Got "VisierModelQueryError" from the above try block
                    throw innerError;
                }
